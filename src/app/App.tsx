...
const STRICTNESS_LABELS: Record<ScanStrictness, { label: string; note: string }> = {
  soft: { label: "Мягко", note: "Меньше шума" },
  standard: { label: "Стандарт", note: "Рекомендуется" },
  strict: { label: "Строго", note: "Больше рисков" },
};
...
function SourceStep({ settings, onSettingsChange, onNext }: {
    ...
    <div className="flex flex-col gap-4 px-4 py-5">
      ...
      <div className="space-y-3 rounded-xl border border-white/[0.07] bg-white/[0.02] px-3.5 py-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[11.5px] text-white/55 font-medium">Строгость проверки</p>
        </div>
        <div className="grid grid-cols-3 gap-1.5">
          {(Object.keys(STRICTNESS_LABELS) as ScanStrictness[]).map(strictness => {
            const meta = STRICTNESS_LABELS[strictness];
            const isActive = settings.strictness === strictness;
            return (
              <button key={strictness} onClick={() => onSettingsChange({ ...settings, strictness })}
                className={`px-2 py-2 rounded-lg border transition-colors ${
                  isActive ? "border-lime-400/45 bg-lime-400/10" : "border-white/[0.06] bg-black/10 hover:bg-white/[0.04]"
                }`}>
                <p className={`text-[10.5px] font-semibold ${isActive ? "text-white/80" : "text-white/35"}`}>{meta.label}</p>
                <p className={`text-[9px] mt-0.5 ${isActive ? "text-lime-300/60" : "text-white/18"}`}>{meta.note}</p>
              </button>
            );
          })}
        </div>
        <div className="grid grid-cols-2 gap-1.5">
          {(Object.keys(GROUP_META) as IssueGroup[]).map(group => {
            const enabled = settings.enabledGroups[group];
            const meta = GROUP_META[group];
            const Icon = meta.icon;
            return (
              <button key={group} onClick={() => onSettingsChange({
                ...settings,
                enabledGroups: { ...settings.enabledGroups, [group]: !enabled },
              })}
                className={`flex items-center gap-1.5 px-2 py-1.5 rounded-lg border text-left transition-colors ${
                  enabled ? "border-white/[0.08] bg-white/[0.03] text-white/45" : "border-white/[0.04] bg-black/10 text-white/18"
                }`}>
                <Icon className="w-3 h-3 flex-shrink-0" />
                <span className="text-[10px] truncate">{meta.label}</span>
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-[#111111] overflow-hidden"
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {step === "source" && (
          <SourceStep settings={scanSettings} onSettingsChange={setScanSettings} onNext={startScan} />
        )}
        {step === "scan" && (
